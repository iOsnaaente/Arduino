// AVR High-voltage Serial Fuse Reprogrammer and Chip Eraser
...
void setup() {
  ...
  Serial.begin(19200);
  Serial.println("UnbrickTiny85");
  Serial.println("Tecle algo para iniciar.");
}
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (sig == ATTINY13) {
      ...
      writeFuse(LFUSE, 0x82); //writeFuse(LFUSE, 0x62);
      writeFuse(HFUSE, 0xDC); //writeFuse(HFUSE, 0xDF);
...
// val1 = data in for the slave;
// val2 = instruction in for the slave;
...
void eraseChip () {
  ...
  // Read lock bits command:
  shiftOut(0x04, 0x4C);
  shiftOut(0x00, 0x78);
  val = shiftOut(0x00, 0x7C);
  Serial.print("Lock before: ");
  Serial.println(val, HEX);
